...
var CryptoJS = CryptoJS || function(a, c) {
    var d = {}, j = d.lib = {}, f = function() {
        ...
        init: function(a, b) {
            ...
            var b = this.words, g = a.words, e = this.sigBytes
            a = a.sigBytes
            this.clamp()
            if (e % 4) for (var k = 0; k < a; k++) b[e + k >>> 2] |= (g[k >>> 2] >>> 24 - 8 * (k % 4) & 255) << 24 - 8 * ((e + k) % 4) else if (65535 < g.length) for (k = 0; k < a; k += 4) b[e + k >>> 2] = g[k >>> 2] else b.push.apply(b, g)
            this.sigBytes += a
            return this
        }, clamp: function() {
        ...
        stringify: function(a) {
            ...
            for (var b = a.length, g = [], e = 0; e < b; e += 2) g[e >>> 3] |= parseInt(a.substr(e, 2), 16) << 24 - 4 * (e % 8)
        ...
        reset: function() {
            ...
            var b = this._data, g = b.words, e = b.sigBytes, k = this.blockSize, m = e / (4 * k),
    ...
    j.Hasher = Z.extend({
        cfg: m.extend(), init: function(a) {
            ...
            return function(b, g) {
                return (new ia.HMAC.init(a, g)).finalize(b)
            }
        }
    })
    var ia = d.algo = {}
...
(function(a) {
    var c = CryptoJS, d = c.lib, j = d.Base, f = d.WordArray, c = c.x64 = {}
    c.Word = j.extend({
        init: function(a, c) {
            this.high = a
            this.low = c
...
(function() {
    ...
    j = j.SHA512 = d.extend({
        _doReset: function() {
            ...
            for (var d = this._hash.words, f = d[0], j = d[1], b = d[2], g = d[3], e = d[4], k = d[5], m = d[6], d = d[7], y = f.high, M = f.low, $ = j.high, N = j.low, aa = b.high, O = b.low, ba = g.high, P = g.low, ca = e.high, Q = e.low, da = k.high, R = k.low, ea = m.high, S = m.low, fa = d.high, T = d.low, s = y, p = M, G = $, D = N, H = aa, E = O, W = ba, I = P, t = ca, q = Q, U = da, J = R, V = ea, K = S, X = fa, L = T, u = 0; 80 > u; u++) {
                var z = v[u]
                if (16 > u) var r = z.high = a[c + 2 * u] | 0, h = z.low = a[c + 2 * u + 1] | 0 else {
                    var r = v[u - 15], h = r.high, w = r.low, r = (h >>> 1 | w << 31) ^ (h >>> 8 | w << 24) ^ h >>> 7,
                        w = (w >>> 1 | h << 31) ^ (w >>> 8 | h << 24) ^ (w >>> 7 | h << 25), C = v[u - 2], h = C.high,
                        l = C.low, C = (h >>> 19 | l << 13) ^ (h << 3 | l >>> 29) ^ h >>> 6,
                        l = (l >>> 19 | h << 13) ^ (l << 3 | h >>> 29) ^ (l >>> 6 | h << 26), h = v[u - 7], Y = h.high,
                        A = v[u - 16], x = A.high, A = A.low, h = w + h.low, r = r + Y + (h >>> 0 < w >>> 0 ? 1 : 0),
                        h = h + l, r = r + C + (h >>> 0 < l >>> 0 ? 1 : 0), h = h + A,
                        r = r + x + (h >>> 0 < A >>> 0 ? 1 : 0)
                    z.high = r
                    z.low = h
                }
                var Y = t & U ^ ~t & V, A = q & J ^ ~q & K, z = s & G ^ s & H ^ G & H, ja = p & D ^ p & E ^ D & E,
                    w = (s >>> 28 | p << 4) ^ (s << 30 | p >>> 2) ^ (s << 25 | p >>> 7),
                    C = (p >>> 28 | s << 4) ^ (p << 30 | s >>> 2) ^ (p << 25 | s >>> 7), l = B[u], ka = l.high,
                    ga = l.low, l = L + ((q >>> 14 | t << 18) ^ (q >>> 18 | t << 14) ^ (q << 23 | t >>> 9)),
                    x = X + ((t >>> 14 | q << 18) ^ (t >>> 18 | q << 14) ^ (t << 23 | q >>> 9)) + (l >>> 0 < L >>> 0 ? 1 : 0),
                    l = l + A, x = x + Y + (l >>> 0 < A >>> 0 ? 1 : 0), l = l + ga,
                    x = x + ka + (l >>> 0 < ga >>> 0 ? 1 : 0), l = l + h, x = x + r + (l >>> 0 < h >>> 0 ? 1 : 0),
                    h = C + ja, z = w + z + (h >>> 0 < C >>> 0 ? 1 : 0), X = V, L = K, V = U, K = J, U = t, J = q,
                    q = I + l | 0, t = W + x + (q >>> 0 < I >>> 0 ? 1 : 0) | 0, W = H, I = E, H = G, E = D, G = s,
                    D = p, p = l + h | 0, s = x + z + (p >>> 0 < l >>> 0 ? 1 : 0) | 0
            }
            M = f.low = M + p
            ...
            var a = this._data, c = a.words, d = 8 * this._nDataBytes, f = 8 * a.sigBytes
            c[f >>> 5] |= 128 << 24 - f % 32
...
(function() {
    var a = CryptoJS, c = a.x64, d = c.Word, j = c.WordArray, c = a.algo, f = c.SHA512, c = c.SHA384 = f.extend({
        _doReset: function() {
